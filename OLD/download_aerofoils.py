...
def download_aerofoils(out_path):
    ...
    for line in response:
        ...
        if line_with_aerofoils:
            for aerofoil_found in line_with_aerofoils:
                ...
                if Path.exists(out_path / (aerofoil_name + ".csv")):
                    continue
                ...
                try:
                    ...
                    for line_data in data:
                        ...
                        if line_with_data:
                            max_ClCd, angle = [float(num) for num in re.findall(r'([+-]?\d*[.]?\d*)', line_with_data.group()) if num != '']
                            break  # we want the first values, which occur at Re = 50,000, Ncrit = 9

                    # print aerofoil file
                    with open(out_path / (aerofoil_name + ".csv"), 'w') as f:
